...
description: >-
  Architecture patterns: client-server, extensions, queries, data access, performance. Companion to ITS std-*. Use for non-trivial design.
disable-model-invocation: true
---

# std-architecture — архитектура конфигурации
...
Determined by `{NEW_OBJECTS_IN}` parameter from ``.1c/project.json``:
...
- Approach depends on `{PLATFORM_VERSION}` from ``.1c/project.json``:
...
### Queries
- Verify metadata attributes (existence, names, types) **before** writing a query — see `поиск по src/ или optional MCP` (rule #3 «verify before writing»; metadata-first via `анализ объекта в src/ (MCP optional)` / `проверка метаданных в src/ (Grep/Read; MCP optional)`).
- Look for existing query examples before writing complex queries (`поиск шаблонов/похожих в src/ (MCP optional)`, `Grep по src/ (MCP optional)`).
...
- Before using any platform API method, verify it exists in `{PLATFORM_VERSION}` from ``.1c/project.json``.
