...
**Remember**: Verify metadata attributes exist using `проверка метаданных в src/ (Grep/Read; MCP optional)` and `чтение XML в src/ (MCP optional)` (for exact types and indexes) before writing queries.
