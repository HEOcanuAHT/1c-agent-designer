...
1. **Verify metadata** with `проверка метаданных в src/ (Grep/Read; MCP optional)` / `чтение XML в src/ (MCP optional)`: do the objects, attributes and tabular sections you assume actually exist? Right now? In the version installed in the project?
2. **Find similar queries** with `Grep по src/ (MCP optional)` / `поиск шаблонов/похожих в src/ (MCP optional)`. Reuse a proven shape rather than inventing one.
