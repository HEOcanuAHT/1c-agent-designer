...
Goal: cut LLM noise and keep only useful comments. See also `skill `coding-standards` → "Comments"` for the headline rule (anchor from `AGENTS.md → Coding Standards`).
