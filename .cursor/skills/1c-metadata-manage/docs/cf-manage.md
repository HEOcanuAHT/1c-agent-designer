...
```powershell
powershell.exe -NoProfile -File .cursor/skills/1c-metadata-manage/tools/1c-cf-manage/scripts/cf-init.ps1 -Name "<Name>" [-OutputDir "<path>"]
```
...
```powershell
powershell.exe -NoProfile -File .cursor/skills/1c-metadata-manage/tools/1c-cf-manage/scripts/cf-edit.ps1 -ConfigPath '<path>' -Operation <op> -Value '<value>'
```
...
```powershell
powershell.exe -NoProfile -File .cursor/skills/1c-metadata-manage/tools/1c-cf-manage/scripts/cf-info.ps1 -ConfigPath "<path>"
```
...
```powershell
powershell.exe -NoProfile -File .cursor/skills/1c-metadata-manage/tools/1c-cf-manage/scripts/cf-validate.ps1 -ConfigPath "<path>"
```
...
When creating a new configuration as part of a project, update SDD artifacts if present (see `related std-* / 1c-forms skills` for detection):
