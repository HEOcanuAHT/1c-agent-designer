...
**Command:**
```powershell
powershell.exe -NoProfile -File .cursor/skills/1c-metadata-manage/tools/1c-role-compile/scripts/role-compile.ps1 -RolePath "<role.json>" -OutputDir "<src-root-or-Roles>"
```
...
```powershell
powershell.exe -File .cursor/skills/1c-metadata-manage/tools/1c-role-info/scripts/role-info.ps1 -RightsPath <path> -OutFile <output.txt>
```
...
```powershell
powershell.exe -NoProfile -File .cursor/skills/1c-metadata-manage/tools/1c-role-validate/scripts/role-validate.ps1 -RightsPath <path> [-MetadataPath <path>] [-OutFile <output.txt>]
```
...
When creating or modifying roles as part of a feature, update SDD artifacts if present (see `related std-* / 1c-forms skills` for detection):
