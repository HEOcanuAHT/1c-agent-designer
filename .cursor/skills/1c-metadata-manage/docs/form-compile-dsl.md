...
> When designing a form from scratch (5+ elements or unclear requirements) — load the canonical `.cursor/skills/1c-forms/docs/form-patterns.md` first (skill-local `form-patterns.md` is a pointer). For simple forms (1–3 fields) it is not needed.
...
```powershell
# Mode 1 — JSON DSL
powershell.exe -NoProfile -File .cursor/skills/1c-metadata-manage/tools/1c-form-compile/scripts/form-compile.ps1 -JsonPath "<json>" -OutputPath "<Form.xml>"
