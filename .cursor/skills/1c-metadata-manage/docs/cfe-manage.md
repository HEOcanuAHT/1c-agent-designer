...
```powershell
powershell.exe -NoProfile -File .cursor/skills/1c-metadata-manage/tools/1c-cfe-manage/scripts/cfe-init.ps1 -Name "МоёРасширение"
```
...
```powershell
powershell.exe -NoProfile -File .cursor/skills/1c-metadata-manage/tools/1c-cfe-manage/scripts/cfe-borrow.ps1 -ExtensionPath src -ConfigPath <config> -Object "Catalog.Контрагенты"
```
...
```powershell
powershell.exe -NoProfile -File .cursor/skills/1c-metadata-manage/tools/1c-cfe-manage/scripts/cfe-diff.ps1 -ExtensionPath src -ConfigPath <config> -Mode A
```
...
```powershell
powershell.exe -NoProfile -File .cursor/skills/1c-metadata-manage/tools/1c-cfe-manage/scripts/cfe-patch-method.ps1 -ExtensionPath src -ModulePath "Catalog.Контрагенты.ObjectModule" -MethodName "ПриЗаписи" -InterceptorType Before
```
...
- **Grep/Read src/ (MCP optional)** — Find objects to borrow and verify module paths.
- **Read object XML (MCP optional)** — Get full object structure for objects being borrowed.
- **search_code** — Find methods to intercept (prefer over `Grep src/ (MCP optional)`; supports semantic/fulltext/hybrid search with detail levels L0–L3).
- **Grep src/ (MCP optional)** — Find methods in raw BSL files (fallback when `search_code` is not available).
- **Grep/Read src/ (MCP optional)** (`names_only=true`) — Find similar metadata objects for extension XML reference.
...
When creating extensions as part of a feature, update SDD artifacts if present (see `related std-* / 1c-forms skills` for detection):
