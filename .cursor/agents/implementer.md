...
4. Новые объекты метаданных **не** создавай (`meta-compile`, `cf-edit add-childObject`, ручной `ChildObjects`). Нужна пустышка из дампа. Нет файла — `Нужно решение`: пользователь создаёт в Конфигураторе, родитель делает dump. Заполнение существующего — `meta-edit` / form-* по ТЗ.
