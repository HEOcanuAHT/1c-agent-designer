...
При правках в `src/_extensions/` дополнительно читай  
...
5. Паттерн перехвата: `&Вместо` → изменить входные данные / ранний выход → при необходимости `ПродолжитьВызов()`. Бизнес-константы — только из ТЗ родителя.
